...
export default function CampaignsContent() {
  ...
  const handleCreateNew = () => {
    router.push("/campaigns/create?back=/campaigns/regular");
  };
  ...
  // Smart polling: Auto-refreshes campaign list when there are sending/pending campaigns
  useEffect(() => {
    const hasActiveCampaign = campaigns.some(
      (c) =>
        c.status?.toLowerCase() === "pending-sending" ||
        c.status?.toLowerCase() === "sending"
    );
  ...
  // ✅ FIXED: now passes token in body too
  ...
  // Function to move well-performing campaigns to regular category
                              ...
                              <div className="flex items-center space-x-1">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    router.push(
                                      `/campaigns/create?edit=${campaign.id}&back=/campaigns/regular`,
                                    );
                                  }}
