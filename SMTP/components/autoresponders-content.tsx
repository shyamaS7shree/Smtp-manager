...
export default function AutoresponderCampaignsContent() {
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", "autoresponders.csv");
  ...
  useEffect(() => {
    ...
    fetchCampaigns().catch((error) => {
      console.error("Failed to fetch latest autoresponders", error);
    });
  }, []);
  ...
  const fetchCampaigns = async () => {
    try {
      const res = await fetch(
        `/api/get-all-campaigns?type=autoresponder&page_number=1&per_page=50`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token()}`,
      ...
      const mappedCampaigns: Campaign[] = records
        .filter((item: any) => String(item?.type || '').toLowerCase() === 'autoresponder')
                        ...
                        >
                          ...
                          <SelectContent className="rounded-xl border-slate-200 shadow-lg">
                            ...
                            <SelectItem value="Active" className="text-xs">
                              <span className="inline-flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-emerald-500"></span>Active</span>
                            </SelectItem>
            ...
            <tbody>
              {(() => {
                ...
                return filteredCampaigns.length > 0 ? (
                  ...
                  <tr>
                    <td
                      colSpan={columns.filter((col) => visibleColumns[col]).length + 1}
